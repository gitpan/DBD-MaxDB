#!perl -w -I./t
#/*!
#  @file           108selectcol_arrayref.t
#  @author         GeroD
#  @ingroup        dbd::MaxDB
#  @brief          use selectcol_arrayref to fetch data
#
#\if EMIT_LICENCE
#
#    ========== licence begin  GPL
...
print " Test 5: call selectcol_arrayref\n";
...
print " Test 7: call selectcol_arrayref with bind_values set\n";
$ref = $dbh->selectcol_arrayref("SELECT i2, i1 FROM GerosTestTable WHERE i1 >= ? AND i2 < ?", undef, 0, 1000) or die "selectall_arrayref failed $DBI::err $DBI::errstr\n";
MaxDBTest::Test(1);
...
$rowindex = 0;
foreach $rowentry (@$ref) {
    my $expectedvalue = ($rowindex + 10);
    if ($rowentry != $expectedvalue) {
        print "wrong data returned: $rowentry. Expected was $expectedvalue.\n";
        $samedata = 0;
    }
    $rowindex++;
}
