#!perl -w -I./t
#/*!
#  @file           121fetchrow_arrayref.t
#  @author         GeroD
#  @ingroup        dbd::MaxDB
#  @brief          check fetchrow_arrayref
#
#\if EMIT_LICENCE
#
#    ========== licence begin  GPL
...
MaxDBTest::beginTest("fetchrow_arrayref and compare (do not fetch all of the data)");
for ($i=0; $i<9; $i++) { # fetch 9 rows
    my $rowref = $sth->fetchrow_arrayref() or MaxDBTest::logerror(qq{fetchrow_arrayref failed $DBI::err $DBI::errstr});
    my ($int, $char) = @$rowref;
...
MaxDBTest::beginTest("check if fetchrow_arrayref fails");
$sth->{'PrintError'} = 0;
...
MaxDBTest::beginTest("fetchrow_arrayref and compare (fetch all of the data)");
